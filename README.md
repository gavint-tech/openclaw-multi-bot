...
คำสั่งจากแผง **Command** ส่งไปที่ `POST /gateway/v1/responses` (ผ่าน nginx) — ต้องเปิด HTTP OpenResponses บน gateway และจัดการ auth ให้ตรง

คู่มือตั้งค่าแบบละเอียด (local + Railway + troubleshooting): **[docs/ui-gateway-connection.md](docs/ui-gateway-connection.md)**

nginx proxy:

- `/gateway/` → `openclaw-gateway:18789` (health, `/v1/responses`, ฯลฯ)
- `/ws` → เดียวกันสำหรับ WebSocket (โปรโตคอลเต็มรูปแบบอยู่ที่เอกสาร OpenClaw — UI ใช้ HTTP เป็นหลักสำหรับคำสั่ง)
